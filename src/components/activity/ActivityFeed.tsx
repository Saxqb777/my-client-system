...
export function ActivityFeed({ activities, showClient = true, emptyHint }: { activities: Row[]; showClient?: boolean; emptyHint?: string }) {
  if (activities.length === 0) {
    return <EmptyState title="No activity yet" hint={emptyHint ?? "Log something with Quick Log and it shows up here."} compact />;
  }
  ...
  return (
    <div className="space-y-5">
      {groups.map((g) => (
        <section key={g.key}>
          <h3 className="eyebrow sticky top-14 z-10 mb-1 bg-[color-mix(in_oklab,var(--bg)_70%,transparent)] py-1 backdrop-blur lg:top-16">{g.label}</h3>
