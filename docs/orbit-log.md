...
- Vercel: team `saxqb777s-projects` (Hobby plan). Project created from the GitHub repo; production branch is the working branch above. Function region sin1.
...
| ORBIT_PASSWORD | Vercel | temporary value set by Claude, Saaqib to change |
| ORBIT_SESSION_SECRET | Vercel | generated by Claude |
| ANTHROPIC_API_KEY | Vercel | Saaqib to add (Orbit falls back to rules until then) |
...
- Saaqib to set ANTHROPIC_API_KEY and change ORBIT_PASSWORD in Vercel.
